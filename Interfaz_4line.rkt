...
(new button% [parent frame]
     [label "Iniciar"]
     [callback (lambda (button event)
                 (if (and (>= (send columnas_4line get-value) 8) (>= (send filas_4line get-value) 8) (<= (send filas_4line get-value) 16) (<= (send columnas_4line get-value) 16) (not gameStart) (not (false? (generarMatriz (send columnas_4line get-value) (send filas_4line get-value)))))
                     (begin
                       (crear_boton1 (send columnas_4line get-value) 0)
                       (matriz_game (generarMatriz (send filas_4line get-value) (send columnas_4line get-value)))
...
;; Se asume que la variable "matrix" está definida y contiene una matriz válida
  (objetivo (reverse (car matrix)) ;; Se invierte la fila para que la PC empiece desde la última columna
                     (length (car matrix)) ;; Se obtiene el alto de la matriz
                      0)) ;; El ultimo argumento es un contador que permite saber cuando se revisaron todas las columnas

;#JUGADA DE LA PC#

;;Función que utiliza el retorno de la IA para mostrar la jugada de la PC
(define (juega_IA_pc)
  (let ((col (solucion))) ;; Se obtiene la columna elegida por la PC
    (case (jugada 2 col (car matrix))
      ...
      (else
       ...
       (writeln (jugada 2 col (car matrix))) ;; Se muestra en la consola la matriz resultante de la jugada de la PC
       (matriz_game (jugada 2 col (car matrix))) ;; Se actualiza la matriz principal con la jugada de la PC
       (actualizacion_tablero) ;; Se actualiza el tablero gráfico con la matriz principal actualizada
       (interfaz_matrix (car matrix))))))

;asigna un valor al jugador que esta en ese momento, si es null es color plata,
; si es jugador(num=1) es del color elegido y si es la IA_pc es de color rosa
...
(define (boton pilar)
  (new button% [parent lugar_boton]   ; Se crea un nuevo botón en el panel "lugar_boton"
             ...
             [callback (lambda (button event)   ; Se crea un procedimiento de callback para el evento click del botón
                         ...
                         (writeln (jugada 1 pilar (car matrix))); Se llama a la función "jugada" con los argumentos 1, "col" y el primer elemento de "matrix". El resultado se imprime en la consola
                         (cond
                           ((equal? (jugada 1 pilar (car matrix)) 1)(ventana_gane))   ; Si el resultado de "jugada" es igual a 1, se llama a la función "ventana_gane"
                           ((equal? (jugada 1 pilar (car matrix)) 2)(ventana_pierde))   ; Si el resultado de "jugada" es igual a 2, se llama a la función "ventana_pierde"
                           ((equal? (jugada 1 pilar (car matrix)) 0) (vetana_empate))   ; Si el resultado de "jugada" es igual a 0, se llama a la función "empate"
                           ((equal? (jugada 1 pilar (car matrix)) (car matrix)) (send button enable #f))   ; Si el resultado de "jugada" es igual al primer elemento de "matrix", se deshabilita el botón
                           (else
                            (matriz_game (jugada 1 pilar (car matrix)))   ; Se llama a la función "matriz_game" con los argumentos 1, "pilar" y el primer elemento de "matrix"
